...
espessura_haste        = 8.0;   
curso_haste            = 110.0; // AJUSTADO PARA O COMPRIMENTO MATEMATICAMENTE PERFEITO
comp_oblongo           = 10.0;  
folga_slot             = 0.4;   
...
// ====================================================================
// Altere esta variável para exportar peça por peça ("base", "alavanca", "haste", "guia_frontal", "botao")
...
module mancal_guia_frontal() {
    ...
    altura_guia_z = z_base_haste + espessura_haste + espessura_teto_guia;
    folga_guia = 0.8;
    largura_bloco = largura_haste_quadrada + 8; 
    comp_base = 36; 

    difference() {
        union() {
            hull() { translate([0, -comp_base/2, 0]) cylinder(r=5, h=h_aba); translate([0, comp_base/2, 0]) cylinder(r=5, h=h_aba); }
            translate([-6, -largura_bloco/2, 0]) cube([12, largura_bloco, altura_guia_z]);
        }
...
if (modo_visualizacao == "montagem") {
    ...
    // Guia Frontal recuada para X = -55 (Evita colisão com a luva do botão)
    color("teal") 
        translate([-65, y_linha_guia_haste, 0]) 
            rotate([180, 0, 0]) 
                mancal_guia_frontal();
